  ...
  return (
    <div className="max-w-7xl mx-auto px-4 lg:px-8 py-10">
      ...
      <div className="grid lg:grid-cols-12 gap-10 lg:gap-12 mb-16 items-start">
        ...
        {/* Details - 6 Columns */}
        <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-6 flex flex-col justify-between">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#7B1E3A] mt-2 mb-3 leading-tight" style={{ fontFamily: 'Playfair Display' }}>
              {product.name}
            </h1>

            <div className="flex items-center gap-3 mb-6 pb-6 border-b border-[#D4AF37]/15">
              <div className="flex items-center gap-1.5 bg-[#FFF8F0] px-2.5 py-1 rounded-lg">
                <StarRating rating={product.rating} size={14} />
                <span className="text-xs font-bold text-[#4A2C2A]">{product.rating}</span>
              </div>
              <span className="text-xs text-[#6B4A48] font-medium">({product.reviews} Verified Customer Reviews)</span>
            </div>

            <div className="flex items-baseline gap-3 mb-6 bg-[#FFF8F0]/50 p-4 rounded-2xl border border-[#D4AF37]/15">
              <span className="text-3xl font-bold text-[#7B1E3A]">₹{product.offerPrice.toLocaleString()}</span>
              <span className="text-base text-[#6B4A48] line-through">₹{product.price.toLocaleString()}</span>
              <span className="ml-auto bg-[#7B1E3A] text-white text-xs font-bold px-3 py-1 rounded-full">
                Save ₹{(product.price - product.offerPrice).toLocaleString()}
              </span>
            </div>

            <div className="mb-8">
              <h4 className="text-xs font-bold uppercase tracking-wider text-[#7B1E3A] mb-2">About This Drape</h4>
            ...
            {/* Specifications Grid */}
            <div className="mb-8">
              <h4 className="text-xs font-bold uppercase tracking-wider text-[#7B1E3A] mb-3">Specifications & Fabric Care</h4>
              <div className="grid grid-cols-2 gap-3">
                {Object.entries(product.specifications || { Fabric: product.fabric, Color: product.color, Length: '6.3 Meters (inc. Blouse)', Care: 'Dry Clean Only' }).map(([k, v]) => (
                  <div key={k} className="bg-white rounded-xl p-3 border border-[#D4AF37]/20 shadow-xs">
                    <p className="text-[10px] uppercase tracking-wider text-[#D4AF37] font-bold m-0 mb-0.5">{k}</p>
            ...
            <div className="flex items-center gap-3 mb-8">
              <button className="btn-golden flex-1 justify-center !py-3.5 !min-h-[50px] !text-sm cursor-pointer shadow-md">
                <FiShoppingBag size={18} /> Buy Now / Add to Bag
              </button>
            ...
            {/* Trust Badges */}
            <div className="grid grid-cols-3 gap-3 p-4 bg-[#FFF8F0] rounded-2xl border border-[#D4AF37]/20 text-center">
              <div className="flex flex-col items-center gap-1">
                <FiTruck className="text-[#2D8F5E] text-lg" />
                <span className="text-[11px] font-bold text-[#4A2C2A]">Free Shipping</span>
                <span className="text-[10px] text-[#6B4A48]">Across India</span>
              </div>
              <div className="flex flex-col items-center gap-1 border-x border-[#D4AF37]/20 px-2">
                <FiAward className="text-[#2D8F5E] text-lg" />
                <span className="text-[11px] font-bold text-[#4A2C2A]">Silk Mark</span>
                <span className="text-[10px] text-[#6B4A48]">100% Certified</span>
              </div>
              <div className="flex flex-col items-center gap-1">
                <FiRefreshCw className="text-[#2D8F5E] text-lg" />
            ...
            {/* Shop info */}
            {shop && (
              <div className="mt-8 card-base p-5 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gradient-to-r from-white via-[#FFF8F0]/30 to-white">
                ...
                <Link to="/shops" className="btn-outline-gold !py-2 !px-5 !min-h-[38px] !text-xs no-underline flex-shrink-0 w-full sm:w-auto text-center">
                  Visit Weaver Store
                </Link>
              </div>
            )}
