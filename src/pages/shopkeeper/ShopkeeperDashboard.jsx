...
const cards = [
  { title: 'Total Masterpieces', value: products.length, icon: <FiPackage size={22} />, color: 'from-[#7B1E3A] to-[#9B2E4A]', change: 'Active Catalogue' },
  { title: 'Weave Categories', value: '8', icon: <FiTrendingUp size={22} />, color: 'from-[#D4AF37] to-[#E8C94A]', change: 'Silk, Cotton, Organza' },
  { title: 'Total Drape Views', value: '1,420', icon: <FiTrendingUp size={22} />, color: 'from-[#2D8F5E] to-[#3AAF6E]', change: '+12% this week' },
  { title: 'Verified Weavers', value: '100%', icon: <FiPackage size={22} />, color: 'from-[#5A1028] to-[#7B1E3A]', change: 'Silk Mark Certified' },
];
        ...
        {cards.map((c, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
            ...
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="text-3xl font-bold text-[#7B1E3A] font-mono tracking-tight m-0">{c.value}</p>
                <p className="text-xs font-bold text-[#6B4A48] uppercase tracking-wider mt-1 m-0">{c.title}</p>
              </div>
              <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${c.color} text-white flex items-center justify-center shadow-md flex-shrink-0`}>
                {c.icon}
              </div>
            </div>
            <div className="pt-3 border-t border-[#D4AF37]/10 flex items-center gap-1.5 text-[11px] text-[#6B4A48]/80 font-medium">
              <FiTrendingUp className="text-[#2D8F5E]" /> {c.change}
            </div>
          </motion.div>
        ))}
      </div>
