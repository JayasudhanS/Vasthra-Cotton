...
export default function ProductCard({ product, index = 0 }) {
  ...
  return (
    <motion.div
      ...
      className="group card-base h-full relative cursor-pointer"
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-3.5 sm:p-4 md:p-5 justify-between bg-white gap-1.5">
        <div>
          ...
          <h3 className="text-[13px] sm:text-sm font-semibold text-[#4A2C2A] mb-1 sm:mb-1.5 line-clamp-1 leading-snug min-h-[1.25rem] group-hover:text-[#7B1E3A] transition-colors" style={{ fontFamily: 'Poppins' }}>
            {product?.name || ''}
          </h3>
        ...
        <div className="flex items-center justify-between border-t border-[#D4AF37]/15 pt-2.5 sm:pt-3 mt-auto min-h-[2.5rem]">
          ...
          {discount > 0 && (
            <span className="text-[10px] sm:text-[11px] text-[#2D8F5E] font-bold bg-[#2D8F5E]/10 px-2 py-0.5 rounded-md">
              {discount}% off
            </span>
          )}
        </div>
